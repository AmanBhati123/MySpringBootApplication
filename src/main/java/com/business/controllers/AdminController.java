...
public class AdminController 
{
	...
	@PostMapping("/donate")
	public ResponseEntity<?> donate(@RequestParam("amount") double amount) {
	    try {
	        ...
	        options.put("receipt", "donate_" + System.currentTimeMillis());
	        options.put("payment_capture", 1); // automatic capture

	        Order order = client.orders.create(options);

	        // response mein order ID, amount, currency send karo
